...
public class RentalService {
    ...
    public void rentVehicle(Driver driver, Vehicle vehicle, LocalDateTime startOfRent) {
        if (driver == null || vehicle == null || startOfRent == null) {
            throw new IllegalArgumentException("Driver, vehicle and startOfRent can not be null");
        }
    ...
    public double returnVehicle(Vehicle vehicle, LocalDateTime endOfRent) throws InvalidRentingPeriodException {
        if (vehicle == null || endOfRent == null) {
            throw new IllegalArgumentException("Vehicle and endOfRent can not be null");
        }
        ...
        double totalPrice = vehicle.calculateRentalPrice(vehicle.getStartRentTime(), endOfRent);

        if (!(vehicle instanceof Bicycle)) {
            int driverFee = vehicle.getDriver().getGroup().getFee();
